...
13. **cgm.lib → cgm.core migration** (**Phase 2** 2026-09-01; Waves 0–5 complete): [`Features/Feature_LibToCore.md`](Features/Feature_LibToCore.md) — first-party inventory, old→new map, shim rules, unittest contract. Timeline: [`Branches/Branch_SpringCleaning.md`](Branches/Branch_SpringCleaning.md). Production `cgm.core` is grep-clean of live first-party `cgm.lib`. First-party modules are parked in **`cgm/libOld`**; `cgm.lib` keeps zoo/ml/bo/openSource. Legacy `cgm/tools` 1.0 UIs deleted; `findTextures` lives in `cgm/core/lib/texture_utils.py`. **Red9 stays** (still maintained). Zoo is unmaintained — own used slices (`SKIN.transfer_fromTo`, `cgm/core/lib/zoo` UI); do not rewrite `baseMelUI` or replace MetaClass. Do **not** revive hollow `attributes`/`search` shims, port `libOld`, or migrate vendored `bo` / `openSource` unless explicitly asked. Update when changing that map.
