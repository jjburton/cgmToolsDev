...
13. **cgm.lib → cgm.core migration** (**complete** 2026-09-01): [`Features/Feature_LibToCore.md`](Features/Feature_LibToCore.md) — first-party inventory, old→new map, shim rules, unittest contract. Timeline: [`Branches/Branch_SpringCleaning.md`](Branches/Branch_SpringCleaning.md). Production `cgm.core` is grep-clean of live first-party `cgm.lib`. First-party modules are parked in **`cgm/libOld`**; `cgm.lib` keeps zoo/ml/bo/openSource. Legacy `cgm/tools` 1.0 UIs deleted; `findTextures` lives in `cgm/core/lib/texture_utils.py`. Do **not** revive hollow `attributes`/`search` shims, port `libOld`, or migrate vendored `zoo` / `ml` / `bo` / `openSource` / Red9 unless explicitly asked (zooSetkey South is a one-line `ml_tools` hook). Update only if changing that map.
