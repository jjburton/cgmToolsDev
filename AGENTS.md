...
6. **cgmSimChain design contract**: [`Features/Feature_SimChain.md`](Features/Feature_SimChain.md) — hair vs cloth attach chains, spline/legacy follow, sim/driven joint contract, broken-chain Details, nCloth preset layering, connect/bake invariants, Query Settings, addEndJoint, **Reload Dependencies** vs **Relaunch Tool**. Update when changing `cgmDynFK`, `dynFKTool`, or `nCloth_utils`. Agent rules: **`cgm-stale-session-diagnosis`**, **`cgm-debug-order`**, **`cgm-simchain-debug-contract`**, **`cgm-agent-token-discipline`**, **`maya-cmds-strings-only`** (with **`cgm-reload-mod`**).
